...
var rformElems = /^(?:textarea|input|select)$/i,
	...
	quickParse = function( selector ) {
		...
		if ( quick ) {
			...
			quick[3] = quick[3] && new RegExp( "(?:^|\\s)" + quick[3] + "(?:\\s|jQuery)" );
		}
		return quick;
	},
...
jQuery.event = {
	...
	remove: function( elem, types, handler, selector, mappedTypes ) {
		...
		for ( t = 0; t < types.length; t++ ) {
			...
			namespaces = namespaces ? new RegExp("(^|\\.)" + namespaces.split(".").sort().join("\\.(?:.*\\.)?") + "(\\.|jQuery)") : null;
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		event.namespace_re = event.namespace? new RegExp("(^|\\.)" + namespaces.join("\\.(?:.*\\.)?") + "(\\.|jQuery)") : null;
